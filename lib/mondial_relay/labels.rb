...
module MondialRelay
  ...
  # - **Texte** — DO NOT use for security code calculation *O*.
  module Labels
    OPERATION = :wsi2_creation_etiquette

    module_function

    # Creates a shipment and returns its number with a label url.
    # @param params [Hash] with the keys provided in a module description.
    #   All mandatory fields **must** be included.
    # @return [Hash] a hash of the following structure:
    #   ```ruby
    #     {
    #       stat: '0',
